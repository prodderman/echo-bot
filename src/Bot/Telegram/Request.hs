...
import           Bot.Telegram.Types     (KeyboardLayout (KeyboardLayout))
...
instance ToJSON KeyboardLayout where
  toJSON (KeyboardLayout layout) = object ["inline_keyboard" .= toJSONList (map makeRow layout)]
    where
      makeRow = map $ \option -> object ["text" .= option, "callback_data" .= option]
...
getUpdates token offset =
  ...
  req GET (https baseUrl /: pack ("bot" ++ token) /: "getUpdates") NoReqBody jsonResponse $
  "offset" =: offset <> "timeout" =: (25 :: Int)
...
sendMessage token chatID message =
  ...
  req
    ...
    mempty
  where
    payload = SendMessageData {chat_id = chatID, text = message}

sendSticker :: String -> Int -> String -> IO (JsonResponse Value)
sendSticker token chatId stickerId =
  runReq defaultHttpConfig $
  req
    POST
    ...
    mempty
  where
    payload = SendStickerData {chat_id = chatId, sticker = stickerId}

sendKeyboardLayout :: String -> Int -> String -> KeyboardLayout -> IO (JsonResponse Value)
sendKeyboardLayout token chatId message options =
  ...
  req
    ...
    mempty
  where
    payload = SendKeyboard {chat_id = chatId, text = message, reply_markup = options}

sendConfirmation :: String -> String -> String -> IO (JsonResponse Value)
sendConfirmation token callbackId text =
  runReq defaultHttpConfig $
  req
    POST
